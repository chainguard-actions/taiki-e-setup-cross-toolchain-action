...
target="${INPUT_TARGET:?}"
# Sanitize user-controlled inputs: strip newlines/carriage returns to prevent
# injection into $GITHUB_ENV and $GITHUB_PATH via derived variables.
          ...
          *)
            ...
            case "${target}" in
              aarch64* | arm64*)
                ...
                qemu_version=$(printf '%s' "${qemu_version}" | tr -d '\n\r')
                install_qemu
                x "${wine_exe}" --version
