...
# Sanitize user-controlled values before they are written to GITHUB_ENV/GITHUB_OUTPUT.
# An attacker-supplied target containing embedded newlines could inject arbitrary
# environment variables into subsequent workflow steps.
target=$(printf '%s' "${target}" | tr -d '\n\r')
target_lower=$(printf '%s' "${target_lower}" | tr -d '\n\r')
target_upper=$(printf '%s' "${target_upper}" | tr -d '\n\r')
